...
#### SANCTIONED broad-catch — per-message dispatch resilience boundary
- A background queue worker's per-message handler (e.g. `MessageSenderService.SendMessageAsync`, called in a loop from `ProcessQueue`) is a SANCTIONED "one poison message must not abort the whole dispatch cycle, nor be retried forever" boundary. The leading `catch (OperationCanceledException) { throw; }` is REQUIRED (shutdown must propagate to `ProcessQueue`'s OCE handler, never mark-failed under a cancelled token). After it, a trailing `catch (Exception ex)` is RETAINED to mark exactly that one message `'failed'` (code-bearing log `OutboundMessageSendCallbackFailed`) so it is neither stranded-and-retried-forever by the FEATURE C `'sending'` recovery sweep nor allowed to break out to `ProcessQueue` and stop the cycle for all other queued messages. `ex.Message` is **log-only / stored in the internal `failed_reason` column** (background worker, no caller-facing response surface) → no client leak. Note the callback transport (`MainAppCallbackClient.SendCallbackAsync`) swallows `HttpRequestException` internally and only throws `OperationCanceledException` on caller-ct cancellation, so an `HttpRequestException` catch here would be DEAD. Do NOT FAIL this trailing broad-catch for CQ "typed catch ONLY". Same class as the auth-login and `/payment/callback` boundaries.
...
## FEAT-INMA-PIPELINE-V2 C4 — 'Set Customer Status' action (sanctioned design, do NOT FAIL)
- **Side-effecting node handler guarded by `ctx.IsSimulation` ONLY:** `ActionSetCustomerStatusHandler` performs a real cxapi write mid-flow (via the Backend proxy). The `INodeHandler` XML doc says "Handlers are PURE — no DB, no HTTP" but that comment is STALE/aspirational — the shipped precedents `ApiCallHandler` (real HTTP) and `EcommerceHandler` (calls Integrations) already do mid-flow I/O guarded ONLY by `ctx.IsSimulation` (mock, no real call), returning `Action=Continue` with `success`/`error` output handles. C4 deliberately matches that exact pattern; `SimulationEngine` (the sole non-prod execution path) sets `isSimulation:true`. Do NOT FAIL for "handler not pure" or "needs a second AllowExternalSideEffects guard" — a divergent guard on one node would be the inconsistency.
- **Backend internal write endpoint trusts body `tenantId` under TWO-layer auth:** `POST /api/internal/customer-feature-groups/update` is on the `/api/internal/` JWT-required prefix (the service JWT's `tenant_id` claim sets `TenantContext`) AND requires `X-Internal-Service-Token` (`IntakeInternalAuth.Validate` constant-time compare vs `InternalServices:SharedSecret`) AND cross-checks `body.tenantId == TenantContext.TenantId` (403 on mismatch). This is service-level + tenant-bound trust, byte-identical to the shipped `/api/internal/leads/intake/wa-direct` pattern. Body tenantId is NOT trusted on its own. Do NOT FAIL as a tenant-isolation gap.
- **Backend -> cxapi direct egress for the WRITE:** `WapCrmFeatureGroupUpdateClient` is the write sibling of the C3b `WapCrmFeatureGroupCatalogClient` and reuses the same whitelisted egress: FIXED `BaseUrl` (never the tenant `ApiUrl` — SSRF), per-request `X-CIB-SecretKey` via `TryAddWithoutValidation` (never `DefaultRequestHeaders`, never logged), `AllowAutoRedirect=false` (301/302 = rate-limit), finite per-attempt timeout. The secret is resolved in Backend ONLY; Automation never holds or sends it. This is NOT a microservice-isolation violation (cxapi is an external vendor; the client lives in `Invekto.Shared`).
- **Deterministic `ClientRequestID` (no GUID):** `invekto-{tenant}-{fg}-{node}-{identity}` is intentionally stable so a deliberate retry of the same write reuses one id. Correctness rests on cxapi FULL-LIST value-idempotency (re-applying the same set = same state) + the C3a FAIL-CLOSED actor guard (a write-back arrives as `actor=='api'` -> suppressed); the `invekto-` prefix echo is a redundant third loop kill. Do NOT FAIL for "non-cryptographic / predictable request id" — it is a correlation key, not a secret.
- **`ExecutionContext.CustomerId/Phone` additive; welcome/cron pass null BY DESIGN:** the inbound orchestrator + the customer-status trigger job populate identity; `TriggerWelcomeFlowJob`/`CronSchedulerJob` keep `phone:null` (they deliberately avoid an extra leads phone lookup in the hot path). A Set Customer Status action in those contexts degrades to the VISIBLE `error` output handle + `INV-AT-090` (logged) — this is an observable, non-silent boundary, not a silent failure.

## FEAT-INMA-PIPELINE-V2 C3 HARDENING — flow-trigger transactional outbox (sanctioned design, do NOT FAIL)
- **`customer_status_flow_outbox` is a SHARED-Postgres table, NOT "another service's DB":** the whole platform runs on ONE shared Postgres (`tenant_registry`, `leads`, `chat_sessions`, `hangfire` are all accessed by multiple services). Backend creates + writes the outbox (in the C2 tx) + drains it; Automation's `TriggerCustomerStatusFlowJob` performs the terminal atomic claim (`ClaimCustomerStatusOutboxAsync`, `UPDATE ... status='done' WHERE status='processing' RETURNING id`). This is **data access to a shared table** — the same way Automation already writes `chat_sessions` and reads `leads`/`chatbot_flows`. It is NOT an `Invekto.<Service>` CODE reference (no project ref added; isolation rule is about code coupling, not shared-DB rows). Do NOT FAIL as "one service directly accessing another's DB".
- **Backend enqueues the Automation job — established G7 pattern:** `CustomerStatusFlowOutboxDrainJob` (Backend) enqueues `Invekto.Automation.Services.Jobs.TriggerCustomerStatusFlowJob` via `IBackgroundJobClient`, exactly as the REMOVED inline C3a dispatch already did. Backend references the Automation job TYPE via `ProjectReference PrivateAssets="all"` (the G7 SCHEDULER HOST EXCEPTION) — args are primitives, no Shared DTO. Do NOT FAIL as a new cross-service coupling; it is a move of the existing one onto a durable source.
- **Drain cross-tenant fetch = sanctioned background sweep (CQ9 exception):** `FetchPendingBatchAsync` claims pending rows across ALL tenants (`FOR UPDATE SKIP LOCKED`), like the precedent `OutboundRepository.FetchPendingOutboxBatchAsync` / `InmaOptOutSyncJob`. This is a background drain, NOT a request path — request-path tenant scoping does not apply. Every claimed row carries its own `tenant_id`, used for the per-tenant `TriggerCustomerStatusFlowJob.ExecuteAsync(tenantId, ...)`; there is no caller-facing cross-tenant leak. Do NOT FAIL for "missing tenant_id filter".
- **Outbox INSERT is tenant-scoped + atomic with the apply:** `InmaWebhookEventRepository.PersistAndApplyAsync` INSERTs the outbox rows with `row.TenantId` (the AUTHENTICATED `TenantContext.TenantId`, never the body companyId) INSIDE the same `NpgsqlTransaction` as the `leads.customer_status` UPDATE, only on `StoredApplied` + only for changed leads (`string.Equals(Ordinal)` = null-safe IS DISTINCT FROM gate) + only when `dispatch.Allowed` (suppression == Fire). A stale-skipped / suppressed / unmatched event writes NO row. This is the transactional-outbox correctness core — do NOT FAIL it as an extra write or a tenant gap.
- **Exactly-once-at-start = at-most-once execution BY DESIGN (not silent failure):** the job claims the outbox row (`processing`->`done`) BEFORE any side effect, so a re-enqueue (drain stale recovery) or racing worker gets 0 rows and skips — preventing a duplicate flow run / duplicate WhatsApp send. A job that crashes AFTER claiming (mid-flow) is NOT retried (the row is already `done`); this is a deliberate, logged at-most-once boundary chosen over duplicate sends. The job's PREFLIGHT (flow lookup) RETHROWS `NpgsqlException` (so Hangfire `AutomaticRetry=2` re-runs a side-effect-free lookup), while the EXECUTION phase keeps swallowing its own infra exceptions (no whole-job retry). Do NOT FAIL the preflight rethrow as inconsistent with the execution swallow — the asymmetry is the whole point (retry-before-effects, never-after).
- **INV-INM-008 repurposed (was inline C3a enqueue fail, now drain enqueue fail):** the inline post-commit enqueue path was removed, so `InmaWebhookFlowEnqueueFailed`/INV-INM-008 now labels the drain's transient enqueue failure (bounced to `pending`); INV-INM-009 (`CustomerStatusFlowOutboxExhausted`) is the terminal `failed` after MaxAttempts. Both are USED (drain `MarkEnqueueFailedAsync` log path). Do NOT FAIL as a dead/unused code or an undocumented semantic drift — errors.md is updated in the same commit.
- **C4 bounded transient retry reverses the prior "no retry" comment ON PURPOSE:** `BackendCustomerStatusClient.UpdateAsync` now retries TRANSIENT classes only (timeout / transport / HTTP 5xx; 2 retries, 500ms then 1500ms awaited backoff). This is safe — NOT a "blind retry of an unknown-outcome mutation" — because the cxapi write is FULL-LIST value-idempotent + the `ClientRequestID` is stable (re-applying = same end state; cxapi dedupes on the id). Business/auth outcomes (4xx, vendor 903/920-923 parsed from the body) take NO retry and go straight to the `error` handle. Do NOT FAIL for "retrying a CRM mutation".
