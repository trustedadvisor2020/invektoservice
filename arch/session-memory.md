...
_Güncel: 2026-06-15 — **FEAT-INMA-PIPELINE-V2 C3 HARDENING = DONE+DEPLOYED+SMOKED.** Flow-trigger durability deliği (C2 commit → C3a enqueue arası crash = trigger kalıcı kaybı; `AutomaticRetry=0` lossy) kapandı. **Transactional outbox:** `customer_status_flow_outbox` (migration 066, UNIQUE tenant_id,event_id,lead_id) — INSERT C2 tx İÇİNDE (`InmaWebhookEventRepository`, StoredApplied + changed lead + suppression==Fire). Backend timer drain (`CustomerStatusFlowOutboxDrainJob`, 10s, FOR UPDATE SKIP LOCKED, `InmaOptOutSyncJob` aynası) → `TriggerCustomerStatusFlowJob` enqueue. **Exactly-once:** job execute öncesi atomik claim (`AutomationRepository.ClaimCustomerStatusOutboxAsync`, processing→done; re-enqueue 0-row skip = çift WhatsApp YOK). **Preflight retry:** flow-lookup NpgsqlException artık THROW + `AutomaticRetry=2`; execution-phase swallow kalır (mid-flow double-send yok). **C4 retry:** `BackendCustomerStatusClient` bounded transient (timeout/transport/5xx, 2×500/1500ms; value-idempotent stable ClientRequestID + full-list-replace). INV-INM-008 repurpose (drain transient enqueue-fail) + INV-INM-009 (exhausted). Commit `c7f26673` (master). **Codex iter0 PASS** (12/12 CQ + 5/5 CoVe + 0 blocking — CODEX UTANSIN ✅); pre-impl codex_consult kritiği at-least-once→exactly-once + preflight-swallow kör noktalarını yakaladı. Migration 066 prod verify (tablo+2 constraint+2 index+grant); Backend+Automation `server-deploy` 10/10 HEALTHY binary-fresh; pipeline INERT (drain boş-tablo no-op). Kalan e2e: Q/INMA + Medipol `webhook_secret`._

_Önceki: **FEAT-INMA-PIPELINE-V2 C4 'Set Customer Status' DONE+DEPLOYED 2026-06-15** (commit `20e8a1c5`, Codex iter2 PASS) — flow ACTION node cxapi `/customer-feature-groups/update` write-back; Backend-proxy (service-JWT+X-Internal-Service-Token → Backend internal → Shared `WapCrmFeatureGroupUpdateClient`); INV-AT-089/090 + INV-BE-140..144; loop-kill C3a actor guard + cxapi value-idempotency. **FEAT-INMA-PIPELINE-V2 5-chunk (C1/C2/C3a/C3b/C4 + C5) + C3 hardening HEPSİ DONE.** Detay → session-memory-archive.md (grep)._

## Recently Completed (max 10)

- **feat-inma-pipeline-v2-c3-hardening** (2026-06-15) — Flow-trigger transactional outbox (migration 066) + exactly-once job-claim + preflight retry + C4 bounded transient retry; commit `c7f26673`; Codex iter0 PASS (12/12 CQ + 5/5 CoVe); Backend+Automation deploy 10/10 HEALTHY binary-fresh, pipeline INERT. Pre-impl codex_consult iter0'ı kurtardı.
...
- **FEAT-INMA-PIPELINE-V2 C4 ✅ DONE+DEPLOYED 2026-06-15** (commit `20e8a1c5`) + **C3 HARDENING ✅ DONE+DEPLOYED 2026-06-15** (commit `c7f26673`, outbox+exactly-once+C4 retry, migration 066, Codex iter0 PASS). Kalan: **canlı e2e smoke** (5050'de durum-değişikliği flow + bir flow'a `action_set_customer_status` node ekleyip cxapi write → INMA echo → C3a suppress) = Q/INMA aksiyonu; **Medipol (100000001) `webhook_secret` set değil** (inbound-echo yarısı). FEAT-INMA-PIPELINE-V2 5-chunk (C1/C2/C3a/C3b/C4 + C5) + C3 hardening HEPSİ DONE.
