...
- **Date:** 2026-04-14
- **Status:** **INMA bridge Faz 1 + Dashboard TS cleanup** her ikisi de Codex PASS + commit. Backend deploy bekliyor (wwwroot SPA assets).
- **Last Task:** (a) Cleanup paketi 185020c — pre-existing TS hatalari (TS6133 unused + TS2322 type) sifir-davranis-degisikligi temizlik. INMA build gate onkosulu. 7 dosya, Codex PASS iter 1 (iter 0 sadece evidence gap). (b) INMA bridge Faz 1 c55957d — src/inma/ izole modul (5 dosya) + App.tsx 3-line useEffect: postMessage handshake, origin whitelist [developer.wapcrm.net, localhost:4200], API_BASE_URL_REGEX, single-flight refresh + 15s timeout, fetch wrapper, TR debug component. INV-INT-100..106 error codes (arch/errors.md kayitli) + inmaErrorMessage() helper. Parent-controlled string ERROR CODE olarak kullanilmaz, sadece diagnostic detail. Codex PASS iter 2 (iter 0: App.tsx cleanup + INV-INT eksik; iter 1: parent.reason leak + UI raw code + EN labels).
- **Build:** Dashboard `npm run build` PASS, tsc 0 hata + vite built 8.27s. .NET solution etkilenmedi.
- **Deploy Status:** Backend redeploy gerekli (wwwroot/app/* SPA assets degisti). Backlog'da.
- **Follow-ups (remaining):** INMA Faz 2 (gercek WapCRM endpoint cagrilari). UP0.2/UP0.3/UP0.5 INMA unblocked backlog. Invekto.Scheduler ayri mikroservis backlog.
- **Next Task:** (1) **Deploy Backend** (wwwroot SPA bundle production'a — 11 tenant). (2) INMA Faz 2 planning. (3) PKT-13 Faz 1 Lead Scoring.
