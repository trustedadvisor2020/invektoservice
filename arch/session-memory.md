...
- **Date:** 2026-04-14
- **Status:** G7 follow-up #1+#3 deployed + **mid-deploy hotfix** (int32-ms cap) + **follow-up #2 shipped & deployed** (Queue attribute routing). Tüm 5 servis + 5 diğer tenant servisi 10/10 healthy. Codex PASS: 3 paket boyunca toplam iter=0/1/0.
- **Last Task:** (a) Hotfix 2199e64 — `TimeSpan.FromDays(36500)` Hangfire int32-ms setter cap'ini (≈24.86d) aşıyor, 3 worker crash-looped; `TimeSpan.FromMilliseconds(int.MaxValue)` ile düzeltildi. Codex PASS iter 1. (b) Follow-up #2 d4b5db3 — 10 job class'ına `[Queue("<service>")]`, 5 Program.cs call-site 4-arg→3-arg. Queue mapping: appointments(3)/automation(3)/backend(2)/integrations(1)/waanalytics(1). Codex PASS iter 0. Migration gerekmedi (attribute dispatch-time okunur).
- **Build:** PASS 0 errors, 18 warnings (baseline).
- **Deploy Status:** Her iki commit de prod'a deploy edildi (5 servis zip+swap). 10/10 health OK.
- **Follow-ups (remaining):** (#4) Backlog: Invekto.Scheduler ayrı mikroservis (§12 compile-only refs tamamen kalksın). INMA unblocked backlog: UP0.2 SSO, UP0.3 Tenant lifecycle, UP0.5 IInmaSendClient.
- **Next Task:** (1) Backlog UP0.2/UP0.3/UP0.5 INMA unblocked olunca. (2) PKT-13 Faz 1 Lead Scoring. (3) Invekto.Scheduler mikroservis planlama (uzun vade).
