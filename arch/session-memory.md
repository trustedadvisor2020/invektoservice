...
| 12b | Zoho OAuth Adim 3 P2 (Sync Orchestrator + Backend Gunes hook + Retry Worker) | NEXT | Yeni session + /auto. Backend AttributionService + FlowEngine lifecycle change noktalarinda HTTP POST /api/internal/zoho/sync (X-Internal-Service-Token header) + ZohoRetryWorker BackgroundService saatlik. |
| 12c | Zoho OAuth Adim 3 P3 (Dashboard UI) | PENDING | ZohoConnectionPage + ZohoStageMappingPage + Sidebar entry. |
