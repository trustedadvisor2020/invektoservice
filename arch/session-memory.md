...
- **Next Task:** (1) **Smoke test**: Dent Adavista tenant ile GET /api/v1/zoho/connect-url -> tarayicida Zoho login -> callback basari sayfasi -> zoho_connections row dogrulama. (2) **Adim 3** Zoho Lead create + Blueprint Transition API + Dashboard "Bagla" butonu + stage mapping UI (yeni session, /auto). (3) Backend wwwroot SPA deploy.
...
| 11 | Zoho OAuth smoke test (Dent Adavista) | PENDING | GET /api/v1/zoho/connect-url -> Zoho login -> callback -> zoho_connections row dogrula |
| 12 | Zoho OAuth Adim 3 (Lead/Blueprint API + Dashboard UI) | PENDING | Yeni session, /auto. Bagla butonu + stage mapping ekrani + Lead create + transition |
