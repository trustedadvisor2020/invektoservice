...
- **Deploy Status:** Production deploy bekliyor: (1) migration 012-zoho-connections.sql calistirilmali (PostgreSQL); (2) Invekto.Integrations servisi redeploy (port 7106); (3) appsettings.Production.json'a Zoho.EU bloku zaten yazili (.env.local + service config'de gitignored); (4) C:\Invekto\Integrations\keys klasor olusturulmali (ilk kez calisirken otomatik creates). Backend wwwroot SPA deploy backlog'da kalmaya devam.
...
- **Next Task:** (1) **Adim 2 deploy** (migration + Integrations servis prod restart, bagli tenant Dent Adavista test). (2) **Adim 3** Zoho Lead/Blueprint API + Dashboard UI (Bagla butonu + stage mapping ekrani). (3) Backend wwwroot SPA deploy.
...
| 10 | Zoho OAuth Adim 2 deploy + Adim 3 (Lead/Blueprint API + UI) | PENDING | Adim 2 Paket A+B commit edildi (de6467c, 6388416), prod migration + servis restart bekliyor; Adim 3 = Lead create + transition + Dashboard UI |
