...
- **Date:** 2026-04-14
- **Status:** G7 §12 scheduler host exception hotfix deployed + verified. Backend.csproj 4 compile-only ProjectReference (PrivateAssets=all) → leader scheduler foreign job types'ı resolve edebiliyor. 7 yeni Succeeded job since 00:13 local (automation:* dahil). Commit f32bf81 pushed master.
- **Last Task:** Scheduler host exception hotfix (Yol A). Backend csproj'a Appointments/Automation/Integrations/WhatsAppAnalytics compile-only refs + System.ServiceProcess.ServiceController 8.0.0→8.0.1 + RemoveReferencedAppsettingsFromPublish Target. §12 spec bölümü + CLAUDE isolation istisna notu + roadmap backlog (Invekto.Scheduler). Worker servisleri de yeniden deploy edildi (62375a2 JobStorage init deploy debt kapandı). Codex iter 0 FAIL (CQ5/CQ9 cross-service ref hard-rule); Q explicit force-commit.
- **Build:** PASS 0 errors, 13 warnings (baseline).
- **Deploy Status:** 5 servis prod'da HEALTHY (Backend + Appointments + Automation + Integrations + WhatsAppAnalytics). 9 recurring job kayıtlı. 4 Hangfire server aktif. §12 validated end-to-end.
- **Follow-ups (known, not blocking):** (1) Worker schedulers race Backend for recurring-jobs lock → stale Error fields when non-§12 scheduler wins; proper fix = disable RecurringJobScheduler on worker servers. (2) `RecurringJob.AddOrUpdate<T>(id, queue, methodCall, cron)` 4-arg overload Queue hash field persist etmiyor; workaround: manuel UPDATE. (3) score=-1 initialization quirk. (4) Roadmap backlog: Invekto.Scheduler ayrı mikroservis.
- **Next Task:** Follow-up 1-3'ü addressleyen bir sonraki paket, VEYA backlog prioritize (UP0.2/UP0.3/UP0.5 — INMA unblocked olunca).
