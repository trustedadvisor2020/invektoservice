...
- **Date:** 2026-04-14
- **Status:** G7 follow-up #1 + #3 implemented + Codex PASS iter 0 (12/12 CQ, 5/5 CoVe). HangfireSetup.AddInvektoHangfire `enableScheduler` optional param (default=true, backward-compat); false → SchedulePollingInterval=100y (RecurringJobScheduler effectively dormant). 4 workers `enableScheduler: false`. Backend startup score=-1 nudge SQL (idempotent, typed-catch non-fatal). Build PASS 0 err/18 warn baseline.
- **Last Task:** Follow-up #1 (Yol B — worker scheduler suppress via SchedulePollingInterval) + #3 (score=-1 startup nudge). Paket slug: `20260414-g7-scheduler-disable-workers`. 6 file, +43/-6. Codex PASS iter 0, commit + push master.
- **Build:** PASS 0 errors, 18 warnings (baseline).
- **Deploy Status:** Commit pushed; prod deploy BEKLİYOR (5 servis: Backend + 4 worker). Deploy sonrası validate: worker server'larda RecurringJobScheduler'ın lock-contention logu kesilmeli; Backend tek scheduler.
- **Follow-ups (remaining):** (#2) `RecurringJob.AddOrUpdate<T>` 4-arg Queue persist quirk — `[Queue("...")]` attribute veya RecurringJobOptions overload. (#4) Roadmap backlog: Invekto.Scheduler ayrı mikroservis.
- **Next Task:** (1) Deploy 5 servis → worker scheduler suppression validate. (2) Follow-up #2 (Queue field persist) ele al. (3) Backlog UP0.2/UP0.3/UP0.5 INMA unblocked olunca.
