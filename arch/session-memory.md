...
- **Date:** 2026-04-15
- **Status:** **Mini paket: quicklogin tenant override (slug 20260415-quicklogin-tenant-override, commit b68f77f)** — POST /api/v1/ops/auth/quicklogin'e opsiyonel `?tenant=<id>` query param; default tenant=0 korundu, `inmaAuthMockEnabled` gate aynen. Yeni INV-OPS-001 (OpsQuickLoginInvalidTenant) Shared/ErrorCodes.cs + arch/errors.md OPS bolumu. Codex iter 1 PASS (12/12 CQ + 5/5 CoVe; iter 0 FAIL false-positive ErrorCodes.GeneralValidation INV-XX-NNN format ihlali — duzeltme: dedicated INV-OPS-001 kodu eklendi). Backend prod deploy + health OK. **Smoke test ertelendi:** quicklogin prod'da gated (inmaAuthMockEnabled=false) — beklendik guvenlik; INMA real login alternatifi denendi (taner@5050) ama 401 her iki INMA env'inde (testapi + api wapcrm). Q smoke'u skip etti, Adim 3'e gectik (yeni sessionda).
- **Last Task (current session):** Mini paket b68f77f — Program.cs quicklogin handler tenant query parsing + INV-OPS-001 validation; Shared/ErrorCodes.cs + arch/errors.md additive (other services unaffected, additive constant binary-compatible). Backend publish + 2 DLL upload (Backend.dll + Shared.dll) + nssm restart InvektoBackend + health 200. NSSM service adi `InvektoBackend` (deploy.md'deki 'Invekto-Backend' yanlis — service exists name var, dash yok). Smoke yapilamadi: quicklogin prod gate'i kapali; INMA login 401 (creds rejected by both wapcrm endpoints).
...
| 12 | Zoho OAuth Adim 3 (Lead/Blueprint API + Dashboard UI + Sync) | NEXT | Yeni session + /auto. ~3 paket. Branch work/20260415-zoho-step3-{slug}. |
