...
- **SS-D DEPLOY (İLK İŞ):** migration 061 prod'a (INV-SEED-061 verifier) → Outbound + Backend + SPA deploy → 10/10 health + `send/pause|resume|cancel` 401 smoke + P0-3 gate re-verify. **Migration ÖNCE** (cancelled_count SELECT kırılması). Kod zaten master'da (`c98b03fc`).
- **PR-4:** approved-template (HSM) send — dil=slug net (G13 çözüldü), statik buton; bulk_send_jobs cxapi kolonları (PR-1'den taşınan CQ11 borcu) + param_mapping per-recipient substitution (modal redesign `{source,column}` shape'ini tüketir). Go-live G12/P0-3 gate'ine bağlı. PR-4 öncesi WapCrmSendClient'ı CANLI cxapi wire-shape'e karşı doğrula (L-2026-06-09 preview-obje dersi).
