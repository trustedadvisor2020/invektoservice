...
| **§3 tenant_profile** | 🟡 KISMEN — **F-VR-B B1 (2026-06-01, commit `47cf8797`):** DATA+CONTRACT katmanı → `voice_tenant_profile` tablosu (business_type + capability flag override + goal/tone, migration 050 + backfill) + Backend `GET /api/ops/tenants/{id}/voice-profile` + Shared `VoiceTenantProfileDto`. VoiceRuntime DB-free → Backend domaininde, HTTP ile okunur. | TÜKETIM henüz yok (B2): VoiceRuntime profil çekme + capability-default RESOLUTION + dinamik prompt. Migration prod'a B2 ile deploy. |
| **§4 sector_adapter** | ✅ **F-VR-B B2 (2026-06-02, Codex iter1 PASS):** `SectorAdapter` registry (6 dil adapter: clinic/dental_clinic/aesthetic_clinic/hair_transplant/ecommerce/generic; 9 key→6 instance, service_business/education/real_estate→generic alias) + `CapabilityDefaults` 9-satır matris (AYRI lookup) + `ResolvedVoiceProfile` (dto.flag ?? default RESOLUTION + sanitized brand_tone/goals) + `VoiceProfileClient` (B1 endpoint HTTP tüketim, graceful-degrade INV-VR-024) + dinamik `InstructionsBuilder` (Persona korunur + SEKTÖR KATMANI: vocabulary/banned/required_fields/capability cümleleri). | intent priority/routing (§7) hâlâ yok (F-VR-D); B2 vocabulary+banned+lead+capability prompt katmanını ekledi |
...
- **F-VR-B — tenant_profile + sector_adapter (config/DB):** §3/§4. Capability flags + sector vocabulary/intent/banned_words/required_fields. Prompt bunlardan dinamik kurulur. **2 chunk:** ✅ **B1 (2026-06-01, commit `47cf8797`)** = DATA+CONTRACT (`voice_tenant_profile` tablosu+migration 050+backfill, Backend `/api/ops/tenants/{id}/voice-profile`, Shared DTO; VoiceRuntime DB-free → Backend domaininde HTTP ile okunur; Codex iter2 PASS). ✅ **B2 (2026-06-02, Codex iter1 PASS)** = VoiceRuntime tüketim: `VoiceProfileClient` (admin-JWT graceful-degrade) + `BusinessType` enum (9) + `SectorAdapter` registry (6 dil adapter, 3 alias→generic) + `CapabilityDefaults` 9-satır matris (AYRI lookup) + `ResolvedVoiceProfile` (dto.flag ?? default + sanitized tone/goals) + dinamik `InstructionsBuilder` (Persona byte-byte korundu, BuildSectorLayer eklendi) + INV-VR-024. VoiceTestContext DEĞİŞMEDİ (ResolvedVoiceProfile Build'e 2. arg). **⚠️ migration 050 deploy Phase 6'da (henüz prod'a uygulanmadı).**
