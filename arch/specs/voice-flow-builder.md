...
| **F2 PBX Live (Toniva) MVP** | 3-4 hafta | **AD-12 Smart-Turn v2 ekle** + 5 yeni flow node + KVKK consent + `call_sessions`/`voice_turns`/`callback_requests` tablo (Migration 050, **VoiceRuntime sahibi AD-15**) + `IVoiceCallProvider` (Toniva + MicrophoneProvider F0'dan + Mock) + Dashboard `/calls` v1 + handoff (REFER) + **AD-13 TTS tenant-selectable** (Azure Neural TR + ElevenLabs Pro DI) + **AD-16 outage hard-fail REFER+callback** | Tek tenant (Dent veya Q-test), tek flow, gercek arama bot cevap |
