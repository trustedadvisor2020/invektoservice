...
**Karar:** Backend `.csproj` dosyasina `Appointments`, `Automation`, `Integrations`, `Marketing` (P10 2026-04-22 eklendi), `WhatsAppAnalytics` icin **compile-only `ProjectReference`** (`PrivateAssets="all"`) eklenir. Bu referanslar:
...
**P10 confirmation (2026-04-22):** P5 FEAT-EFS (Marketing servisine FollowupStageJob eklendi) kurulumunda bu checklist ATLANDIGI icin Marketing.csproj `ProjectReference`'i Backend.csproj'a eklenmedi. P9 pilot smoke S7b FAIL raporu prod forensic ile `FileNotFoundException: Could not resolve assembly 'Invekto.Marketing'` hatasini expose etti. Fix: commit `ac390e7` Marketing ref eklendi, `a6ed4ea` transitive `System.ServiceProcess.ServiceController` 8.0.1→9.0.0 bumped. **Checklist for adding a new Hangfire service** (hot take): (1) Service.csproj: `Hangfire.AspNetCore` + `Hangfire.PostgreSql` package refs, (2) Program.cs: `AddInvektoHangfire(queueName, connectionString, enableScheduler)` with `enableScheduler: false` unless this service is the leader, (3) Job class'a `[Hangfire.Queue("<queueName>")]` attribute (class-level for pure job types, method-level for service classes with non-Hangfire methods), (4) **Backend.csproj'a `<ProjectReference Include="..\Invekto.<Service>\Invekto.<Service>.csproj" PrivateAssets="all" />` ekle (ZORUNLU — atlanirsa JobLoadException/FileNotFoundException Scheduled→Enqueued promotion'da surfaces)**, (5) appsettings.Production.json'a `ConnectionStrings:PostgreSQL` (Hangfire fallback via HangfireSetup.ResolveConnectionString), (6) Transitive package downgrade NU1605 olursa Backend'in ilgili package ref'ini bump et. Adim 4 en kolay unutulandir.
