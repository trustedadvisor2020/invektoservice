...
public class InmaOptOutSyncJobTests
{
    ...
    public void TransactionalEventRegistry_AllowList_ExactMatchOnly()
    {
        ...
        Chatinbox.Shared.Services.TransactionalEventRegistry.IsTransactional("appointment_confirmed_en")
            .Should().BeTrue();
        Chatinbox.Shared.Services.TransactionalEventRegistry.IsTransactional("offer_sent_consult_tr")
            .Should().BeTrue();
        Chatinbox.Shared.Services.TransactionalEventRegistry.IsTransactional("offer_sent_spam")
            .Should().BeFalse();
        Chatinbox.Shared.Services.TransactionalEventRegistry.IsTransactional("APPOINTMENT_CONFIRMED_EN")
            .Should().BeFalse();   // case-sensitive
        Chatinbox.Shared.Services.TransactionalEventRegistry.IsTransactional(null)
            .Should().BeFalse();
        Chatinbox.Shared.Services.TransactionalEventRegistry.IsTransactional("")
            .Should().BeFalse();
    }
}
