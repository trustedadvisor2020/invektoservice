...
import type { CodexReviewInput, CodexReviewResult, TokenUsage } from "./types.js";
...
function resolveDiffContent(args: CodexReviewInput): string {
...
async function handleCodexReview(
  ...
  if (resolvedDiff.length < MIN_DIFF_LENGTH) {
    throw new Error(
      `Diff content is empty or too short (${resolvedDiff.length} chars, minimum ${MIN_DIFF_LENGTH}). ` +
  ...
  const resolvedArgs: CodexReviewInput = { ...args, git_diff: resolvedDiff };
...
async function main() {
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      const input = args as unknown as CodexReviewInput;
