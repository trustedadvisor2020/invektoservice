 ...
 */
const UNKNOWN_CQ = { result: "UNKNOWN", evidence: "" };
// ─── Main Parser ──────────────────────────────────────────────
export function parseCodexResponse(responseText, questions, modelUsed, tokenUsage) {
...
function heuristicCQ(text, num) {
    const pattern = new RegExp(`CQ${num}[^\\n]*?(PASS|FAIL|UNKNOWN)`, "i");
    const match = text.match(pattern);
    if (match) {
        return { result: match[1].toUpperCase(), evidence: "(heuristic parse)" };
    }
    return UNKNOWN_CQ;
}
// ─── CoVe Parser ──────────────────────────────────────────────
function parseCoVe(text, questions) {
    const results = {};
...
function heuristicCoVe(text, id) {
    const pattern = new RegExp(`${id}[^\\n]*?(PASS|FAIL|UNKNOWN)`, "i");
    const match = text.match(pattern);
    if (match) {
        return { result: match[1].toUpperCase(), reasoning: "(heuristic parse)" };
    }
    return { result: "UNKNOWN", reasoning: "" };
}
// ─── Verdict Parser ───────────────────────────────────────────
function parseOverallVerdict(text, cqGate, coveResults) {
