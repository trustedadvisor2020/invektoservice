...
function getMethodColor(method: string): string {
  switch (method) {
    ...
    case 'POST': return 'bg-brand-50 text-brand-700';
    ...
    <>
      <Card>
        <CardContent className="space-y-2">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className={`w-7 h-7 rounded-lg flex items-center justify-center ${
                service.status === 'ok' ? 'bg-emerald-50 text-emerald-600' :
          ...
          {/* Error message */}
          {service.error && (
            <div className="p-1.5 bg-red-50 border border-red-100 rounded text-[10px] text-red-600 truncate">
              {service.error}
            </div>
          )}
          ...
          <div className="bg-white rounded-2xl shadow-elevated max-w-lg w-full max-h-[80vh] overflow-hidden border border-navy-100">
            ...
            <div className="p-4 border-b border-navy-100 flex items-center justify-between">
              ...
              <button
                ...
                className="w-8 h-8 rounded-lg hover:bg-navy-50 flex items-center justify-center text-navy-300 hover:text-navy-600 transition-colors"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
              ...
              {loading ? (
                ...
                <div className="space-y-4">
                  {sortedCategories.map(category => (
                    <div key={category}>
                      <h4 className="text-xs font-semibold text-navy-300 uppercase tracking-wider mb-2 px-1">
                        {category}
                      </h4>
                      <div className="space-y-1.5">
                        {groupedEndpoints[category].map((endpoint, idx) => (
                          <div key={idx} className="p-3 rounded-lg bg-navy-50/60 hover:bg-navy-50 transition-colors group">
                            <div className="flex items-start justify-between gap-3">
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2 mb-1">
                                  ...
                                  <span className="font-mono text-sm text-navy-700 truncate">{endpoint.path}</span>
                                </div>
                                <div className="flex items-center gap-2">
                                  <p className="text-xs text-navy-400">{endpoint.description}</p>
                                  {endpoint.auth && endpoint.auth !== 'none' && (
                                    <span className="px-1 py-0.5 rounded text-[10px] bg-amber-50 text-amber-600 border border-amber-100">
                                      {endpoint.auth}
                                    </span>
                                  )}
                                </div>
                              ...
                              {endpoint.method === 'GET' && (
                                <a
                                  ...
                                  className="opacity-0 group-hover:opacity-100 transition-opacity p-1.5 rounded hover:bg-navy-100"
                                >
