...
using Chatinbox.Backend.Endpoints;
using Chatinbox.Backend.Services;
using Chatinbox.Backend.Services.Jobs;
...
using Chatinbox.Shared.DTOs.Payment;
using Chatinbox.Shared.DTOs.Translation;
using Chatinbox.Shared.Logging;
using Chatinbox.Shared.Logging.Reader;
...
builder.Services.AddSingleton(sp => new Chatinbox.Shared.Contracts.Inma.WapCrmFeatureGroupCatalogClient(
...
builder.Services.AddSingleton<Chatinbox.Shared.Services.WapCrmFeatureGroupCatalogCache>();
...
builder.Services.AddSingleton(sp => new Chatinbox.Shared.Contracts.Inma.WapCrmFeatureGroupUpdateClient(
...
// preserve scope/indentation and minimise the diff.
{
    ...
    var inmaWebhookOptions = new Chatinbox.Backend.Services.Inma.InmaWebhookOptions();
    builder.Configuration.GetSection(Chatinbox.Backend.Services.Inma.InmaWebhookOptions.SectionName).Bind(inmaWebhookOptions);
    ...
    var flowOutboxOptions = new Chatinbox.Backend.Services.CustomerStatusFlowOutboxDrainOptions();
    ...
    builder.Services.AddHostedService<Chatinbox.Backend.Services.CustomerStatusFlowOutboxDrainJob>();
    ...
    builder.Services.AddSingleton<Chatinbox.Backend.Services.Photos.IPhotoInboundHandler,
        Chatinbox.Backend.Services.Photos.PhotoInboundHandler>();
...
builder.Services.AddSingleton<Chatinbox.Shared.Contracts.Inma.IInmaDynamicFieldsClient>(sp =>
{
    ...
    return new Chatinbox.Shared.Contracts.Inma.HttpInmaDynamicFieldsClient(
        sp.GetRequiredService<IHttpClientFactory>().CreateClient("inma_dynamicfields"),
        sp.GetRequiredService<JsonLinesLogger>(),
        baseUrl);
});
builder.Services.AddSingleton<Chatinbox.Shared.Services.InmaDynamicFieldsCache>();
...
builder.Services.AddSingleton<Chatinbox.Backend.Data.TenantSettingsRepository>();
builder.Services.AddSingleton<Chatinbox.Shared.Contracts.TenantFieldMapping.DbTenantFieldMappingResolver>();
builder.Services.AddSingleton<Chatinbox.Shared.Contracts.TenantFieldMapping.ITenantFieldMappingResolver>(sp =>
    sp.GetRequiredService<Chatinbox.Shared.Contracts.TenantFieldMapping.DbTenantFieldMappingResolver>());
...
builder.Services.AddSingleton<Chatinbox.Shared.Contracts.Campaigns.DbTenantCampaignResolver>();
builder.Services.AddSingleton<Chatinbox.Shared.Contracts.Campaigns.ITenantCampaignResolver>(sp =>
    sp.GetRequiredService<Chatinbox.Shared.Contracts.Campaigns.DbTenantCampaignResolver>());
...
builder.Services.AddSingleton<Chatinbox.Shared.Contracts.ClinicMetadata.DbTenantClinicMetadataResolver>();
builder.Services.AddSingleton<Chatinbox.Shared.Contracts.ClinicMetadata.ITenantClinicMetadataResolver>(sp =>
    sp.GetRequiredService<Chatinbox.Shared.Contracts.ClinicMetadata.DbTenantClinicMetadataResolver>());
...
builder.Services.AddSingleton<Chatinbox.Backend.Services.MetaLeadgen.MetaLeadgenConfigRepository>();
builder.Services.AddSingleton<Chatinbox.Backend.Services.MetaLeadgen.MetaLeadgenEventRepository>();
builder.Services.AddSingleton<Chatinbox.Backend.Services.MetaLeadgen.MetaLeadgenWebhookService>();
builder.Services.AddSingleton<
    Chatinbox.Backend.Services.MetaLeadgen.IMetaGraphApiClient,
    Chatinbox.Backend.Services.MetaLeadgen.MetaGraphApiClient>();
builder.Services.AddHttpClient(
    Chatinbox.Backend.Services.MetaLeadgen.MetaGraphApiClient.HttpClientName,
    c =>
    {
...
app.MapGet("/ops/debug2", async (HttpContext ctx, LogReader logReader) =>
{
    ...
    // Try LogReader query
    var queryResult = await logReader.QueryLogsAsync(new Chatinbox.Shared.Logging.Reader.LogQueryOptions
    {
...
app.MapPost("/api/ops/services/{serviceName}/restart", async (HttpContext ctx, string serviceName) =>
{
    ...
    {
        "Chatinbox.Backend" => "InvektoBackend",
        "Chatinbox.ChatAnalysis" => "InvektoChatAnalysis",
        "Chatinbox.Automation" => "InvektoAutomation",
        "Chatinbox.AgentAI" => "InvektoAgentAI",
        "Chatinbox.Outbound" => "InvektoOutbound",
        "Chatinbox.Knowledge" => "InvektoKnowledge",
        "Chatinbox.Appointments" => "InvektoAppointments",
        "Chatinbox.Integrations" => "InvektoIntegrations",
        "Chatinbox.WhatsAppAnalytics" => "InvektoWhatsAppAnalytics",
        "Chatinbox.Marketing" => "InvektoMarketing",
        "Chatinbox.WebChat" => "InvektoWebChat",
        _ => null
    };
...
app.MapPost("/api/v1/webhook/event", async (HttpContext ctx, JsonLinesLogger jsonLogger) =>
{
    ...
    var inmaWebhookOptions = ctx.RequestServices.GetService<Chatinbox.Backend.Services.Inma.InmaWebhookOptions>()
        ?? new Chatinbox.Backend.Services.Inma.InmaWebhookOptions();
    ...
    if (msgLogRepo != null)
    {
        ...
        var photoHandler = ctx.RequestServices.GetService<Chatinbox.Backend.Services.Photos.IPhotoInboundHandler>();
        if (photoHandler == null || pgFactory == null)
        {
            ...
            if (webhookEvent.Messages.Any(m => Chatinbox.Backend.Endpoints.InmaInboundMediaEndpoint.IsMediaMessage(m)))
            {
                jsonLogger.SystemWarn(
        ...
        else
        {
            ...
            foreach (var msg in webhookEvent.Messages)
            {
                if (!Chatinbox.Backend.Endpoints.InmaInboundMediaEndpoint.IsMediaMessage(msg))
                    continue;

                var phone = Chatinbox.Backend.Endpoints.InmaInboundMediaEndpoint.ExtractPhoneFromChatId(msg.ChatId);
    ...
    {
        var sigHeader = ctx.Request.Headers[Chatinbox.Backend.Services.Inma.InmaWebhookSignatureValidator.SignatureHeader].FirstOrDefault();
        var tsHeader = ctx.Request.Headers[Chatinbox.Backend.Services.Inma.InmaWebhookSignatureValidator.TimestampHeader].FirstOrDefault();
        var eventIdHeader = ctx.Request.Headers[Chatinbox.Backend.Services.Inma.InmaWebhookSignatureValidator.EventIdHeader].FirstOrDefault();
        ...
        if (!Chatinbox.Backend.Services.Inma.InmaWebhookSignatureValidator.Verify(secret, tsHeader, body, sigHeader, inmaWebhookOptions, out var matchedEncoding))
        {
    ...
    static bool IsTimestampFresh(string? ts, Chatinbox.Backend.Services.Inma.InmaWebhookOptions opts)
    {
...
app.MapGet("/api/ops/postman", async (HttpContext ctx) =>
{
    ...
    var postmanPath = Path.Combine(AppContext.BaseDirectory, "postman", "Chatinbox.postman_collection.json");
    ...
    ctx.Response.Headers.ContentDisposition = "attachment; filename=\"Chatinbox.postman_collection.json\"";
...
// PrivateAssets="all" ProjectReference to Chatinbox.Automation, Chatinbox.Appointments,
// Chatinbox.Marketing, Chatinbox.Integrations, Chatinbox.WhatsAppAnalytics. Hangfire
...
//
// Therefore: trigger lives here (Backend), NOT in Chatinbox.Appointments. Appointments
// service stays Chatinbox.Shared-only (csproj has only Chatinbox.Shared ref). The same
        ...
        try
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (leadId.HasValue)
                    {
                        BackgroundJob.Schedule<Chatinbox.Automation.Services.Jobs.PhotoRequestDispatchJob>(
...
app.MapPost("/api/internal/customer-feature-groups/update", async (
    ...
    IConfiguration config,
    JsonLinesLogger jsonLog,
    [FromServices] Chatinbox.Shared.Contracts.Inma.WapCrmFeatureGroupUpdateClient updateClient,
    [FromServices] TenantRegistryRepository tenantRepo) =>
{
    ...
    var updateReq = new Chatinbox.Shared.Contracts.Inma.Dtos.WapCrmFeatureGroupUpdateRequest
    {
    ...
    try
    {
        ...
        switch (result.Outcome)
        {
            case Chatinbox.Shared.Contracts.Inma.Dtos.WapCrmFeatureGroupUpdateOutcome.Success:
                jsonLog.SystemInfo(
            ...
            case Chatinbox.Shared.Contracts.Inma.Dtos.WapCrmFeatureGroupUpdateOutcome.AuthRejected:
                jsonLog.SystemWarn(
            ...
            case Chatinbox.Shared.Contracts.Inma.Dtos.WapCrmFeatureGroupUpdateOutcome.BusinessError:
            default:
                jsonLog.SystemWarn(
    ...
    catch (Chatinbox.Shared.Contracts.Inma.WapCrmFeatureGroupUpdateException ex)
    {
        jsonLog.SystemWarn(
...
app.MapGet("/api/ops/tenants/{id:int}/voice-profile", async (HttpContext ctx, int id, JsonLinesLogger jsonLog) =>
{
    ...
    try
    {
        ...
        if (profile == null)
        {
            ...
            profile = new Chatinbox.Shared.DTOs.Voice.VoiceTenantProfileDto
            {
                TenantId = id,
...
app.MapGet("/api/v1/dynamic-fields", async (
    HttpContext ctx,
    JsonLinesLogger jsonLog,
    [FromServices] Chatinbox.Shared.Services.InmaDynamicFieldsCache cache,
    [FromServices] TenantRegistryRepository tenantRepo) =>
{
    var requestId = ctx.Request.Headers["X-Request-Id"].FirstOrDefault() ?? Guid.NewGuid().ToString("N");
    ...
    catch (Chatinbox.Shared.Contracts.Inma.InmaDynamicFieldsFetchException ex)
    {
        jsonLog.StepWarn(
...
app.MapPost("/api/v1/dynamic-fields/cache-invalidate", (
    HttpContext ctx,
    JsonLinesLogger jsonLog,
    Chatinbox.Shared.Services.InmaDynamicFieldsCache cache) =>
{
    var requestId = ctx.Request.Headers["X-Request-Id"].FirstOrDefault() ?? Guid.NewGuid().ToString("N");
...
app.MapGet("/api/v1/customer-feature-groups", async (
    HttpContext ctx,
    JsonLinesLogger jsonLog,
    [FromServices] Chatinbox.Shared.Services.WapCrmFeatureGroupCatalogCache catalogCache,
    [FromServices] TenantRegistryRepository tenantRepo) =>
{
    var requestId = ctx.Request.Headers["X-Request-Id"].FirstOrDefault() ?? Guid.NewGuid().ToString("N");
    ...
    catch (Chatinbox.Shared.Contracts.Inma.WapCrmFeatureGroupCatalogFetchException ex)
    {
        jsonLog.StepWarn(
...
app.MapPost("/api/v1/customer-feature-groups/cache-invalidate", (
    HttpContext ctx,
    JsonLinesLogger jsonLog,
    Chatinbox.Shared.Services.WapCrmFeatureGroupCatalogCache catalogCache) =>
{
    var requestId = ctx.Request.Headers["X-Request-Id"].FirstOrDefault() ?? Guid.NewGuid().ToString("N");
...
Chatinbox.Backend.Services.MetaLeadgen.MetaLeadgenEndpoints.MapMetaLeadgenEndpoints(app);
...
Chatinbox.Backend.Services.Lifecycle.LifecycleInternalEndpoints.MapLifecycleInternalEndpoints(app);
...
Chatinbox.Backend.Endpoints.InmaInboundMediaEndpoint.MapInmaInboundMediaEndpoint(app, internalApiKey);
Chatinbox.Backend.Endpoints.PhotoEndpoints.MapPhotoEndpoints(app);
...
// ops-only; callers either post JSON directly or rely on future Chatinbox.Shared DTO if
...
namespace Chatinbox.Backend { public partial class Program { } }
