...
using Chatinbox.Integrations.Services;
using Chatinbox.Integrations.Services.Jobs;
...
using Chatinbox.Shared.DTOs.Integrations;
using Chatinbox.Shared.DTOs.Reviews;
...
builder.Services.AddSingleton<Chatinbox.Integrations.Services.Video.GoogleMeetMockProvider>();
builder.Services.AddSingleton<Chatinbox.Integrations.Services.Video.VideoProviderFactory>();
...
builder.Services.AddSingleton<Chatinbox.Integrations.Services.Ikas.IkasTokenManager>(sp =>
{
    var httpClient = sp.GetRequiredService<IHttpClientFactory>().CreateClient(ikasTokenHttpClient);
    return ActivatorUtilities.CreateInstance<Chatinbox.Integrations.Services.Ikas.IkasTokenManager>(sp, httpClient);
});
builder.Services.AddSingleton<Chatinbox.Integrations.Services.Ikas.IkasGraphQlClient>(sp =>
{
    var httpClient = sp.GetRequiredService<IHttpClientFactory>().CreateClient(ikasGraphQlHttpClient);
    return ActivatorUtilities.CreateInstance<Chatinbox.Integrations.Services.Ikas.IkasGraphQlClient>(sp, httpClient);
});
builder.Services.AddSingleton<Chatinbox.Integrations.Services.Ikas.IkasProvider>();
builder.Services.AddSingleton<IEcommerceProvider>(sp =>
    sp.GetRequiredService<Chatinbox.Integrations.Services.Ikas.IkasProvider>());
    ...
    string? cargoProvider) =>
{
    ...
    if (events.Count > 0)
    {
        var cached = new Chatinbox.Shared.DTOs.Integrations.CargoTrackingResponse
        {
            TrackingCode = trackingCode,
            CargoProvider = cargoProvider ?? "unknown",
            CurrentStatus = events[^1].Status,
            Events = events.Select(e => new Chatinbox.Shared.DTOs.Integrations.CargoTrackingEvent
            {
                Status = e.Status,
                Location = e.Location,
    ...
    if (!string.IsNullOrEmpty(cargoProvider))
    {
        ...
        var response = new Chatinbox.Shared.DTOs.Integrations.CargoTrackingResponse
        {
            TrackingCode = result.TrackingCode,
            CargoProvider = result.CargoProvider,
            CurrentStatus = result.CurrentStatus,
            Events = result.Events.Select(e => new Chatinbox.Shared.DTOs.Integrations.CargoTrackingEvent
            {
                Status = e.Status,
                Location = e.Location,
                EventTime = e.EventTime
...
// Chatinbox.Appointments VideoMeetingCreationJob. Header-secret auth (no JWT) —
// tenant_id travels in the body because the caller is a Hangfire job without
// a HttpContext-bound TenantContext.
Chatinbox.Integrations.Endpoints.VideoMeetingEndpoints.Map(app);
...
logger.SystemInfo($"Chatinbox.Integrations starting on port {listenPort}");
app.Run();
