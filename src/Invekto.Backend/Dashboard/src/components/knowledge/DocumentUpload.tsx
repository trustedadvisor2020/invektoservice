...
export function DocumentUpload({ tenantId, open, mode, onClose, onUploadComplete }: Props) {
  ...
  /** Normalize bare domain to https:// URL */
  ...
  /** Extract domain from user input for display */
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (mode === 'pdf') {
        ...
        await api.indexWebsite(tenantId, normalizeUrl(url), title || undefined);
      }
      reset();
      onUploadComplete();
      onClose();
