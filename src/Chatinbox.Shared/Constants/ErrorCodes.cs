...
public static class ErrorCodes
{
    ...
    public const string InmaWebhookFlowEnqueueFailed = "INV-INM-008";     // WARN (drain, transient): FEAT-INMA-PIPELINE-V2 C3 HARDENING — CustomerStatusFlowOutboxDrainJob failed to enqueue TriggerCustomerStatusFlowJob (BackgroundJobClientException / NpgsqlException) for a claimed outbox row; the row is bounced back to 'pending' (attempts < max) and retried on the next tick. No trigger is lost (the outbox row survives). Replaces the removed C2->C3a inline post-commit enqueue path.
    public const string CustomerStatusFlowOutboxExhausted = "INV-INM-009"; // WARN (drain, terminal): FEAT-INMA-PIPELINE-V2 C3 HARDENING — an outbox row's enqueue retries hit MaxAttempts; status flips to 'failed' (ops repair signal). The status was already applied to the lead in C2; only the automation dispatch is stuck — re-driveable by resetting the row to 'pending'.
    ...
    // FEAT-OBI Phase 2 — Telefon Numarası Ara (single-number history, INV-OB-097+)
    public const string PhoneHistoryInvalidNumber = "INV-OB-097";          // Phone-history lookup/export: phone empty or not normalizable to a valid E.164 number (PhoneNormalizer returned null). No query runs. 422.
