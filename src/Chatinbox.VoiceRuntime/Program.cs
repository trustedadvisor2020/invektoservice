...
using Chatinbox.VoiceRuntime.Audio;
using Chatinbox.VoiceRuntime.Endpoints;
using Chatinbox.VoiceRuntime.Metrics;
using Chatinbox.VoiceRuntime.Providers;
using Chatinbox.VoiceRuntime.Realtime;
...
// ── Logging ───────────────────────────────────────────────────────
var logger = new JsonLinesLogger("Chatinbox.VoiceRuntime", logPath);
...
builder.Services.AddTransient<Chatinbox.VoiceRuntime.Clients.TenantInfoClient>();
builder.Services.AddTransient<Chatinbox.VoiceRuntime.Clients.FlowInfoClient>();
builder.Services.AddTransient<Chatinbox.VoiceRuntime.Clients.KnowledgeSearchClient>();
...
builder.Services.AddTransient<Chatinbox.VoiceRuntime.Clients.VoiceProfileClient>();
...
builder.Services.AddSingleton(
    Chatinbox.VoiceRuntime.Tools.KbConfidenceOptions.FromConfig(builder.Configuration, logger));
builder.Services.AddTransient<Chatinbox.VoiceRuntime.Tools.SearchKnowledgeBaseTool>();
...
if (!File.Exists(vadModelPath))
{
    var msg = $"[{ErrorCodes.VoiceRuntimeVadModelMissing}] [Boot] Silero VAD model missing at '{vadModelPath}'. " +
              $"Run download script from src/Chatinbox.VoiceRuntime/Models/README.md.";
...
logger.SystemInfo($"[Boot] Chatinbox.VoiceRuntime starting on port {listenPort} (env={app.Environment.EnvironmentName})");
