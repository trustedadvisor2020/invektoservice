...
using Chatinbox.Automation.Services;
using Chatinbox.Automation.Services.Jobs;
using Chatinbox.Shared.Hosting;
using Chatinbox.Shared.Middleware;
using Chatinbox.Shared.Services;
using Chatinbox.Automation.Services.NodeHandlers;
...
using Chatinbox.Shared.DTOs.Returns;
...
builder.Services.AddSingleton<Chatinbox.Shared.Contracts.TenantFieldMapping.ITenantFieldMappingResolver,
    Chatinbox.Shared.Contracts.TenantFieldMapping.DbTenantFieldMappingResolver>();
builder.Services.AddSingleton<Chatinbox.Shared.Services.DynamicMessageValidator>();
...
builder.Services.AddSingleton<Chatinbox.Shared.Contracts.Campaigns.ITenantCampaignResolver,
    Chatinbox.Shared.Contracts.Campaigns.DbTenantCampaignResolver>();
...
builder.Services.AddSingleton<Chatinbox.Shared.Contracts.ClinicMetadata.ITenantClinicMetadataResolver,
    Chatinbox.Shared.Contracts.ClinicMetadata.DbTenantClinicMetadataResolver>();
...
builder.Services.AddSingleton<Chatinbox.Shared.Services.ITemplateRotationService,
    Chatinbox.Shared.Services.HashBasedTemplateRotationService>();

// HFM-1: chunk planner (stateless jittered schedule)
builder.Services.AddSingleton<Chatinbox.Shared.Services.IMessageChunkPlanner,
    Chatinbox.Shared.Services.MessageChunkPlanner>();
...
builder.Services.AddSingleton<Chatinbox.Automation.Services.Photos.IPhotoRequestService,
    Chatinbox.Automation.Services.Photos.PhotoRequestService>();
builder.Services.AddScoped<Chatinbox.Automation.Services.Jobs.PhotoRequestDispatchJob>();
builder.Services.AddScoped<Chatinbox.Automation.Services.Jobs.PhotoRequestReminderJob>();
builder.Services.AddScoped<Chatinbox.Automation.Services.Jobs.PhotoEscalationJob>();
...
builder.Services.AddTransient<Chatinbox.Automation.Services.Jobs.NoReplyCheckJob>();
...
builder.Services.AddHttpClient(
    Chatinbox.Automation.Services.Lifecycle.LifecycleBackendClient.HttpClientName,
    client =>
    {
        client.BaseAddress = new Uri(backendBaseUrl);
        client.Timeout = TimeSpan.FromSeconds(5);
...
builder.Services.AddSingleton<
    Chatinbox.Automation.Services.Lifecycle.ILifecycleBackendClient>(sp =>
{
    var factory = sp.GetRequiredService<IHttpClientFactory>();
    return new Chatinbox.Automation.Services.Lifecycle.LifecycleBackendClient(
        factory.CreateClient(Chatinbox.Automation.Services.Lifecycle.LifecycleBackendClient.HttpClientName),
        backendIntakeSharedSecret,
        sp.GetRequiredService<JwtGenerator>(),
        sp.GetRequiredService<JsonLinesLogger>());
});
...
builder.Services.AddSingleton(
    new Chatinbox.Automation.Services.Jobs.MetaLeadgenSharedSecretHolder(backendIntakeSharedSecret));
builder.Services.AddTransient<Chatinbox.Automation.Services.Jobs.MetaLeadgenIntakeJob>();
...
// Response shape mirrors FlowLookupResponse in Chatinbox.Shared.Contracts.Leads.
// ============================================================
    ...
    HttpContext ctx, string? name, AutomationRepository repo, JsonLinesLogger jsonLogger, CancellationToken ct) =>
{
    ...
    try
    {
        ...
        var body = new Chatinbox.Shared.Contracts.Leads.FlowLookupResponse
        {
