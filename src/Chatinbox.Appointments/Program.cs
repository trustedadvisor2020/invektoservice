...
using Chatinbox.Appointments.Services;
using Chatinbox.Appointments.Services.Jobs;
    ...
    AppointmentBookRequest? request) =>
{
    ...
    catch (NpgsqlException ex)
    {
        jsonLogger.SystemWarn(
            $"[{Chatinbox.Appointments.Services.Video.VideoHopErrorCodes.VideoMeetingHopFailed}] " +
    ...
    // (Backend.csproj zaten Chatinbox.Automation PrivateAssets='all' ref var — G7 SCHEDULER
    ...
    AppointmentCancelRequest? request) =>
{
    ...
    {
        jsonLogger.SystemWarn(
            $"[{Chatinbox.Appointments.Services.Video.VideoHopErrorCodes.VideoReminderSkippedStateChanged}] " +
