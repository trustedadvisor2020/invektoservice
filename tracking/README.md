...
| FEAT-INMA-PIPELINE-V2 | INMA-Authoritative customer_status (Zoho-out + flow channel + flow action) | Q karari 2026-05-12: Zoho Blueprint INSE'den TAMAMEN çıkıyor (lead CRUD + stage sync + Blueprint + COQL + OAuth + `zoho_*` tabloları hepsi silinir). INMA agent UI'da `customer_status` field manuel değiştirilir. **Akış:** (1) INMA agent dropdown → status değişir; (2) INMA → INSE webhook `POST /api/v1/inbound/inma/customer-status-change` (INMA contract yazacak, biz consumer); (3) INSE opaque `customer_status TEXT` saklar, validate ETMEZ; (4) Flow Builder'a 4. trigger kanalı `customer_status_changed`; (5) Yeni action node `Set Customer Status` (INSE → INMA POST geri). **5 chunk:** C1 Zoho-out (Integrations servisi büyük çapta küçülür — `Services/Zoho/*` + ZohoConnectionPage/ZohoStageMappingPage/ZohoSyncLogPage SPA + zoho_* tablolar drop migration) + C2 inbound endpoint (HMAC + idempotency) + C3 flow trigger + C4 flow action + C5 dashboard cleanup. **Bağımlılık:** INMA contract gelmesi (C2-C4 BLOCKED), C1 (Zoho silme) bağımsız ilerler. **Pre-flight:** Faz A runtime fix'leri (A1-A6) önce. | [Karar memory](../../../Users/taner/.claude/projects/c--CRMs-InvektoServices/memory/project_inma_pipeline_v2_decision.md) — spec/plan DRAFT bekliyor | DRAFT (Faz A sonrası) | 12 May | - |
...
**Toplam:** 53 paket (28 done, 0 in-progress, 5 planned, 3 pending, 11 feature DRAFT/SPEC, 1 pilot blocked, 0 active customer, 3 backlog, 2 cancelled) | 50+ GR + RI-100+ task
