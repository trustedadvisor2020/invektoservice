...
| FEAT-TFM | Tenant Field Mapping | MVP: tenant_settings.field_mapping JSONB + DbTenantFieldMappingResolver (5dk cache + single-flight) + Validator (reserved guard INV-BE-096..099) + Backend GET/PUT /api/v1/tenant-settings/field-mapping + DI swap 3 servis (Null→Db). UI editor + INMA mirror sync + flow builder picker → sonraki paketler. | [Spec](../arch/features/tenant-field-mapping.md) / [Plan MVP](../arch/plans/20260421-feat-tfm-resolver-mvp.json) / [Detay](feat-tfm-mvp.md) | MVP IN_PROGRESS 21 Nis | 21 Nis | pending |
