...
| FEAT-LIW | Lead Intake Webhook | Chunk A: Generic POST /leads/intake + field map + dup merge. Chunk B: TriggerWelcomeFlowJob real dispatch + /api/internal/leads/intake/wa-direct + AutomationOrchestrator hook. Chunk C: Dashboard UI (API key manager + field map editor + dry-run preview + flow rename warning). | [Spec](../arch/features/lead-intake-webhook.md) | A+B DONE+DEPLOYED+SMOKED 18 Nis 23:35 UTC; Chunk C UI PLANNED | 18 Nis | A iter 4 PASS / B iter 3 → Q FORCE PASS |
