...
| FEAT-INMA-PIPELINE-V2 | INMA-Authoritative customer_status (Zoho-out + flow channel + flow action) | Q karari 2026-05-12: Zoho Blueprint INSE'den TAMAMEN çıkıyor. INMA otorite. **5 chunk:** **C1 Zoho-out DONE+DEPLOYED+SMOKED 2026-05-12 17:50 UTC (commit `0c0733b`)** — Migration 048 atomik tx snapshot-then-drop 11 tenant data preserved (live 1+2+0 → archive 1+2+0); Invekto.Integrations Services/Zoho/ 22+3 dosya + Invekto.Backend Services/Zoho/ 9 dosya + Dashboard SPA pages/zoho/ + components/zoho/ + zoho-store silindi; Shared Contracts/Zoho/ 8 DTO silindi; arch/errors.md INV-INT-110..137 + FE-131/132 block delete (43 kod); 7 cross-service comment generalization; Codex chunked review Chunk A FAIL (cross-chunk visibility) + Chunks B-E MCP server error → Q FORCE PASS (A3+A4a precedent); 10/10 service /health post-deploy; Hangfire 0 zoho orphan jobs (G4 natural cleanup validated). C2 inbound endpoint + C3 flow trigger + C4 flow action **BLOCKED** (INMA contract bekliyor). C5 dashboard cleanup + DentAdavista/plan refresh PENDING (post-C1). | [Plan](../arch/plans/20260513-feat-inma-pipeline-v2-c1-zoho-out.json) / [Tracking](feat-inma-pipeline-v2-c1.md) / [Memory](../../../Users/taner/.claude/projects/c--CRMs-InvektoServices/memory/project_inma_pipeline_v2_decision.md) | C1 DONE+DEPLOYED+SMOKED; C2-C4 BLOCKED; C5 PENDING | 12 May | C1 Q FORCE PASS (Codex MCP infra issue) |
