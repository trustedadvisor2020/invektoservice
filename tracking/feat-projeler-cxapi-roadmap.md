...
> **Durum:** IN PROGRESS (PR-1/PR-2/PR-3a DONE; PR-3b BLOCKED-G12; PR-4 TODO; **PKT-14 Projeler S1+S2+S3+S4 ✅ DONE — paket TAMAM + PROD'A DEPLOY EDİLDİ 2026-06-07** [migration 055+056+057 canlı, 10/10 HEALTHY, GATE korundu]) · **Oluşturma:** 2026-06-06 · **Sahip:** Q
...
- **G12 (🔴 BLOCKER — hâlâ açık):** cxapi `/chatoperation` response `requestID`, delivery-status webhook'taki id ile **birebir aynı mı**? + webhook payload şeması + firma/instance kimliği var mı + callback **nereye** route'lanıyor (bizim webhook'a mı, köprüye mi)? (ext_id eşleşmesi + P0-3 gate buna bağlı — teyit alınmadan delivery-status migration yapılmaz.) **INMA cevabı 2026-06-08 (kısmî):** yalnızca *mekanizma* teyit edildi → durum **anlık webhook push** + **okundu/read aktif** (dokümante değil ama çalışıyor). Korelasyon anahtarı / tam payload şeması / firma kimliği / routing / `wamid` HÂLÂ açık → **anonim örnek payload bekleniyor.**
