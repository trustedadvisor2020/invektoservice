...
> **Slug:** feat-vfb | **Spec:** [SPEC-008](../arch/specs/voice-flow-builder.md) | **Mod:** F0 LIVE / F0.5 DEFERRED (prod=F0 voice-poc, repo=F0.5; F0.5 dropdownlar 3-defect cascade ile bloke — aşağı bak)
> **Versiyon:** v2.2 | **Olusturma:** 2026-05-17 | **Son Guncelleme:** 2026-05-31 (Backend recovered + 4 fix DEPLOYED [stale-binary/embed/WS-Http1/OpenAI-format] + Automation Chunk E deployed; F0.5 browser dropdownlar ERTELENDİ — host+exposure+token-rol 3-defect; prod F0'a revert edildi voice testable)
...
- [x] **Chunk E backend/CORS DEPLOYED** (2026-05-31, commits `62a5f613`+`af00f498`, Q OVERRIDE — Codex MCP down): Backend master HEAD redeploy (stale-binary root cause çözüldü — kaynak doğruydu, RDF crash 5-satır minimal-repro ile teşhis: body-less endpoint'te SADECE DI-kayıtsız complex param crash eder, Backend'in 28 offender'ı hepsi kayıtlı) → `/api/ops/tenants` VoicePocCors CANLI (super.invekto.com preflight 204+ACAO). Automation master HEAD deploy → `/api/v1/flows` VoicePocCors CANLI. VoiceRuntime: format fix + Http1 + embed. **Önceki "INCIDENT 9+ suspect endpoint" YANLIŞ teşhisti — stale binary'di, source-fix gerekmedi.**
- [⏸️] **F0.5 browser dropdownlar ERTELENDİ (3-defect cascade, Chunk D/E hiç e2e test edilmemiş):** (1) **HOST** — voice-poc.js `BACKEND_BASE/AUTOMATION_BASE = app.invekto.com` (IIS, /api/ops/tenants 404); ops API `super.invekto.com` (Kestrel) serve ediyor. (2) **EXPOSURE** — Automation:7108 hiçbir external host'tan erişilemez (super+app /api/v1/flows 404) → browser-direct imkansız, **Backend flow-proxy gerekir** (Backend zaten `/api/v1/flow-builder/flows/{tid}` ile Automation proxy'liyor, VoicePocCors eklenmeli). (3) **TOKEN ROL** — voice JWT `role=service`, `/api/ops/tenants` ValidateOpsAuth `role==admin AND TenantId==0` istiyor (D027 audit) → reddedilir. **FIX PLAN (next session, stabil infra + Codex up):** (a) Backend'e voice için tenant+flow listeleyen CORS'lu ops endpoint(ler), browser→Backend→Automation isolation pattern, (b) voice-poc.js `BACKEND_BASE=super.invekto.com` + flows Backend proxy path, (c) token-rol kararı (voice-jwt admin mintle VEYA ValidateOpsAuth service-token kabul — auth-sensitive), (d) Backend+VoiceRuntime redeploy + e2e smoke. **prod=F0 (voice-poc git pre-Chunk-D `b67e6747~1`'e revert, dropdown yok, DefaultConfig + format fix → voice testable), repo=F0.5 (bekliyor).**

**Exit:** AC1-AC6 PASS + Codex iter ≤ 2 per chunk + smoke (Q live mikrofon test, KB sorgu round-trip <2sn p95) + 3 servis redeploy HEALTHY (Knowledge unchanged, F4 widget gate). **Şu an: code+doc Exit Criteria PASS, deploy Exit Criteria FAIL — Backend recovery sonrası tamamlanır.**
