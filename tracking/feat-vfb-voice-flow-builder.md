...
- [!] **Chunk E** PARTIAL (code+doc DONE commit `46e4e01a` Codex iter 1 PASS, **PROD DEPLOY BLOCKED by Backend incident**): CORS allowed origins — Backend + Automation Program.cs `VoicePocCors` named policy (`voice.invekto.com:8443`, GET-only) + endpoint `.RequireCors("VoicePocCors")` (`/api/ops/tenants` + `/api/v1/flows/{tenantId}`) + voice-runtime-ws.md Chunk D additive (URL-bridge JWT + HUD rendering rules + CORS prereq) + tracking güncel + AD-35/36. **🚨 INCIDENT (2026-05-27 00:05-00:42):** Backend 4 deploy × 4 crash — `System.InvalidOperationException: Body was inferred but the method does not allow inferred body parameters`. Root cause: .NET 8.0.19 RequestDelegateFactory stricter inspection + Backend Program.cs 245 endpoint × ~20 conditional-registered service tipi implicit DI param'larda startup crash. 3 hotfix attempt (cacheRepo [FromServices] + pgConnectionString unconditional block + 8 endpoint [FromServices] for Codex-flagged suspects) yetmedi, 9+ suspect endpoint kaldı. Hotfix attempts `git stash@{0}` (recoverable). Failed-20260527-000759 server'da artifact. **Backlog:** Q manuel triage — systematic 245-endpoint audit + bulk [FromServices] strategy karar + Backend recovery + Chunk E resume (Automation + VoiceRuntime deploy + Voice Test smoke).
