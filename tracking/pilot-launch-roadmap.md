...
| 10 | FEAT-EFS Hangfire Marketing-Followup Queue Fix | `20260423-feat-efs-hangfire-queue-fix` | **IN_PROGRESS** 2026-04-23 code done, build PASS, Codex review pending | Marketing + Backend | **Root cause confirmed via prod forensic (NOT the 3 hypothetical options):** Orphan `default` queue. Two job registrations (`ApiKeyRateLimiter.SweepNow` + `FollowupStageJob`) lack explicit queue routing → Hangfire defaults to `"default"` queue → no server listens to it (5 servers all on named queues) → 1019 stuck rows since 2026-04-18 (4-day latent bug). **Fix:** (a) `[Queue("marketing-followup")]` class attribute on `FollowupStageJob`; (b) `[Queue("backend")]` method attribute on `ApiKeyRateLimiter.SweepNow`; (c) Backend startup WARN guard probe `hangfire.jobqueue WHERE queue='default' > 0` (early-detection for future regressions). **Cleanup:** Post-deploy `DELETE FROM hangfire.jobqueue WHERE queue='default'` (1019 orphan rows). **Re-smoke:** P9 S7b+S8 fake Dent lead + EFS trigger + test_mode=true → stage[0] Succeeded + event_followup_runs.status='sent' + `[FEAT-EFS][SEND-INTENT]` log. |
