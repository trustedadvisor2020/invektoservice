...
| **B-VCP-DOCTORS** | **VCP Meeting Hop — `doctors` Tablo Schema/Code Drift Fix** | **Paket A AC4 escalation 2026-04-24:** `AppointmentsRepository.cs:1199-1202` SQL'i `LEFT JOIN doctors d ON d.id = s.doctor_id` içeriyor — PostgreSQL parse-time tablo varlığı zorunlu (LEFT JOIN bile olsa). Hem prod DB'de hem `arch/db/migrations/` altında doctors tablo migration'ı YOK. Latent reveal: Dent ilk gerçek Appointments slot booking yapan tenant — daha önce hiç tetiklenmemiş code path. Hangfire job retry loop sürerse ops noise (10 retry × N booking). Pilot meeting_link populate hop için BLOCKER — Dent gerçek müşteri booking'inde meeting_link gelmez. Scope (önerilen): (a) Migration 031 doctors tablo (id+tenant_id+name+is_active boilerplate); (b) AppointmentsRepository.cs SQL guard analysis (LEFT JOIN guard veya doctors_view fallback); (c) Backend.csproj Appointments PrivateAssets="all" pattern verify (G7 SCHEDULER HOST EXCEPTION refleks); (d) S6 re-smoke meeting_link non-null + 2 reminder Hangfire job IDs set. **Risk:** MEDIUM (migration + SQL guard + service deploy + re-smoke). **Activation gate:** Paket C öncesi (pilot meeting flow için şart) — Q onayı + doctors entity domain spec confirm (multi-doctor mu, tek-doctor mu — Dent pilot kapsamı tek-doctor yeterli). |
...
| Metric | Value |
|--------|-------|
| Total Pilot-Critical Packages | 12 (P10 P9-escalation 2026-04-22 + P11 post-P9 wiring 2026-04-22 + Paket A 2026-04-24) |
| DONE | 12 (P1-P6 DEPLOYED+SMOKED; P7 retroactive; P8 prod file op; P9 DONE+SMOKED_PARTIAL; P10 EFS Hangfire fix; P11 FlowBuilder wiring; **A Dent plan_tier upgrade 2026-04-24 11:25 UTC**) |
| IN_PROGRESS | 0 |
| PENDING | 0 |
| SKIPPED | 0 |
| Progress | 100% pilot-critical paketler (12/12); pilot go-live için sıradaki: Paket C (template content bind 46 welcome + 36 FAQ) + B-VCP-DOCTORS (meeting_link populate hop fix). |
| Backlog Packages | 7 (B0/B1/B2/B3/B4/B5/**B-VCP-DOCTORS** new 2026-04-24 — Paket A AC4 escalation) |
