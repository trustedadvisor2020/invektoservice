...
| **B-META** | **Meta Leadgen Webhook Native Entegrasyon** | Q karar 2026-04-24 17:30 UTC: Zapier/3rd-party bridge YOK, Invekto içinde Meta Lead Ads webhook adapter. Scope: (a) `GET`+`POST /api/inbound/meta/leadgen/{tenantId:int}` endpoint + HMAC-SHA256 constant-time signature validate + Hangfire `MetaLeadgenIntakeJob` (queue=automation) + Graph API lead fetch + LIW hop, (b) migration 033 `tenant_settings.meta_leadgen_config` JSONB + NEW `meta_leadgen_events` table (7-day retention + UNIQUE tenant+leadgen_id idempotency), (c) error codes `INV-META-001..006` yeni namespace + postcondition `INV-SEED-018..020`, (d) cross-service Automation→Backend `/api/internal/lifecycle/welcome-sent` HTTP hop (MarketingFollowupClient precedent) + `ZohoLifecycleDispatcher.DispatchEvent(welcome_sent)` hook (TriggerWelcomeFlowJob `messages.Count > 0`), (e) Dashboard SPA `/settings/meta-leadgen` (verify_token rotate + app_secret/page_access_token/page_id + field_id_map editor + [Keşfet: Form Fields] + [Test Events]). Plan `arch/plans/20260425-feat-meta-leadgen-webhook.json` PLANNING 2026-04-24 17:45 UTC. Q override max_diff_lines=200 → ~760 (atomik paket, pilot 3-gün timeline). | **PROMOTED 2026-04-24 17:45 UTC** (in active development, plan JSON DRAFT, Phase 2 Dev next session). Pilot Stage 1 launch direct dependency — bu paket commit edildikten sonra müşteri Meta App + HSM + Zoho Blueprint hazırlıklarıyla paralel yürür. |
...
| Metric | Value |
|--------|-------|
| Total Pilot-Critical Packages | 14 (P10 P9-escalation 2026-04-22 + P11 post-P9 wiring 2026-04-22 + Paket A 2026-04-24 + Paket B B-VCP-DOCTORS 2026-04-24 + Paket C1 Dent FAQ Surface Content Bind 2026-04-24) |
| DONE | 14 (P1-P6 DEPLOYED+SMOKED; P7 retroactive; P8 prod file op; P9 DONE+SMOKED_PARTIAL; P10 EFS Hangfire fix; P11 FlowBuilder wiring; A Dent plan_tier upgrade; B B-VCP-DOCTORS Doctors Bootstrap; **C1 Dent FAQ Surface Content Bind 2026-04-24 18:00 UTC DONE+DEPLOYED+SMOKED**) |
| IN_PROGRESS | 0 |
| PENDING | 0 |
| SKIPPED | 0 |
| Progress | **100% pilot-critical paketler (14/14)**; pilot go-live için tüm content ve mimari hazır. AI FAQ runtime authority (`faqs` 36 row + 36 NOT NULL embedding) çalışır, flow welcome DocX persona-rich content (Güneş + Dr. Özge + Dublin/Cork). Backlog: B-C2 welcome templates overhaul (post-pilot), B0 FEAT-VCP Chunk C Google Meet OAuth (Q-provision). |
| Backlog Packages | 7 (B0/B1/B2/B3/B4/B5 + **B-C2 welcome overhaul post-pilot** Q G13 split decision) |
