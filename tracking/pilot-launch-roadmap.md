...
| **A4b** | WhatsAppAnalytics `Hangfire.JobStorage` DI Path | `20260512-a4b-waa-jobstorage` | WhatsAppAnalytics/Program.cs:277 `EnsureJobStorageInitialized` çağrısı `mssqlConfigured` flag'ine bağlı; stderr 2026-05-06 boot crash'i "No service for type 'Hangfire.JobStorage' has been registered". Hipotez: `AddInvektoHangfire` ile `EnsureJobStorageInitialized` flag tutarsızlığı veya config dependency. Investigate + minimal fix. | 1-2 saat → 15dk | **DONE+VALIDATED (NO-CODE-FIX) 2026-05-12 17:43 UTC** — Investigate kanıtladı: stderr 11x `HangfireSetup.cs:line 99` referansı PRE-A4a binary (commit `62375a2` era, EnsureJobStorageInitialized line 99'daydı). A4a deploy 2026-05-12 16:44:54'te yeni Invekto.Shared.dll (line 127) + WhatsAppAnalytics.dll bütünüyle yenilendi. Mevcut Program.cs:42,226,248,277,294 hepsi aynı `if (mssqlConfigured)` guard'ında — fiziksel mismatch imkansız. Lessons-learned 2026-04-14 entry (satır 377) bu deseni "AddInvektoHangfire ile EnsureJobStorageInitialized AYNI guard'a sarılmalı" diye çoktan yakalamıştı. **Smoke proof:** Faz A queue protokolü gereği defensive restart-smoke uygulandı (Q seçimi): pre-state stderr 524771 byte LastWriteTime 16:49:00 (50dk crashsız) → nssm stop graceful → stderr+stdout truncate (3 byte BOM) → nssm start 17:42:46 → +15s wait → `/health` 200 + `/ready` 200 (DB) + stderr 3 byte (CLEAN) + stdout 3 byte (CLEAN) + PID 11628 uptime 32s CPU 1.33s. **Doc-only paket**, kod değişikliği yok, deploy yok. |
...
| **A5** | Demo Retry (Q ile birlikte) | manuel | Q tenant 5050 prod URL üzerinden Dashboard navigation. Login + flow-builder + translate + zoho-connection HEPSI 200 OK (A3 JWT fix + A2 discovery fix prod path'inde live confirmed). Sadece "Bilgi Bankası" sayfası 401 INV-AU-002 — frontend-only error (api.ts:1154), root cause Backend Knowledge endpoint'leri Ops-admin-only. | 30dk | **DONE+VALIDATED** 2026-05-12 14:04-14:14 UTC (canlı browser + jsonl trail) |
| **A6** | Knowledge endpoints dual-auth (A5'ten çıkan Bilgi Bankası fix'i) | `20260512-a6-knowledge-dual-auth` | Backend `/api/ops/knowledge/{tenantId}/*` 5 helper gate'e (KnProxyGet/Post/Put/Delete + inline upload) JWT-tenant-match fallback eklendi. Ops-admin path UNTOUCHED (true branch korundu), false branch'e `ValidateJwtTenantAuth(ctx, tenantId)` short-circuit fallback. Normal user `TenantContext.TenantId == path tenantId` ile kendi knowledge'ına erişir. Templates Catalog (tenantId=0) için fallback yine yalnız SuperAdmin TenantId=0 JWT'ye açık (D027 scheme). Frontend api.ts DOKUNULMADI — path migration yerine backend gate genişletme, SPA build/deploy gerek yok. | 1 dosya / 36 ins / 5 del / Codex iter 0 PASS 12/12 CQ + 7/7 CoVe + 0 blocker (CODEX UTANSIN ✅) | **DONE+DEPLOYED+SMOKED** 2026-05-12 14:28:32 UTC (commit `3dbfae4`). Backend solo deploy + 3/3 smoke PASS: (S1) no-auth → 401 regression; (S2) tenant=5050 JWT + path=5050 → 200 + real data (invekto.com document 58 chunks); (S3) cross-tenant probe (5050 JWT + path=18173130) → 401 (tenant guard enforced). |
...
- **C3 Flow Builder customer_status_changed trigger** ✅ DONE+DEPLOYED 2026-06-13 (C3a backend trigger `d9a2ecc6` + C3b UI node/catalog dropdown `08cbb9c2`; Codex PASS iter0).
- **C4 Flow Builder Set Customer Status action node** ✅ DONE+DEPLOYED 2026-06-15 (commit `20e8a1c5`, Codex iter2 PASS; Backend-proxy → cxapi `/customer-feature-groups/update`; Backend+Automation 10/10 HEALTHY, endpoint 401-gated; CANLI ama inert; live e2e = Q/INMA + Medipol `webhook_secret`). **FEAT-INMA-PIPELINE-V2 5-chunk (C1/C2/C3a/C3b/C4 + C5) HEPSİ DONE.**
