...
| — | Meta Conversions API (CAPI) | EVALUATED | [arch/features/meta-conversions-api.md](../arch/features/meta-conversions-api.md) + [tracking/feat-meta-capi.md](feat-meta-capi.md) — Server-side donusum bildirimi (Lead/Schedule/Purchase/CompleteRegistration). Marketing servisi (:7112) Hangfire queue, EMQ icin hashed PII + FBC/FBP cookie + IP/UA, browser Pixel ile event_id deduplication. INMA bypass. ROI: $10k/ay reklam spend uzerinde %10-15 efficiency. 5 chunk, ~4-6 session. PENDING Q Pixel/Token provision. |
| — | Meta Ads Insights (read-only) | EVALUATED | [arch/features/meta-ads-insights.md](../arch/features/meta-ads-insights.md) + [tracking/feat-meta-ads-insights.md](feat-meta-ads-insights.md) — `ads_read` permission ile reklam performans Dashboard widget (spend/impressions/clicks/CTR/CPM + lead matching). Async report API + 1 saat cache + Hangfire daily refresh. CAPI ile bundle App Review onerisi. 5 chunk, ~3-4 session. |
| — | Meta Marketing API (campaign create/manage) | IDEA | Reklam kampanya olusturma/butce yonetimi/raporlama Invekto Dashboard icinden. **GATE:** `ads_management_standard_access` Meta App Review barı $10k toplam spend ile gecmez (anlamli volume + tutarli use-case ister). Once CAPI + Ads Insights production'da olmali. Activation gate: spend tabani $50k+/ay veya tier-degistirici musteri talebi. Spec yazimi backlog. |
| — | Zoho Direct Connection Sunset (Inse -> INMA bridge) | IDEA | INMA Zoho entegrasyonunu kurduktan sonra Invekto Services tarafindaki dogrudan Zoho baglantisi kaldirilacak. Tum Zoho veri iletisimi INMA uzerinden olacak (chatoperation tek-kopru pattern'inin Zoho'ya uyarlamasi). **GATE:** INMA Zoho entegrasyonu production'da stabil + smoke test PASS. **Kapsam:** (1) `zoho_connections` + `zoho_stage_mappings` + `zoho_sync_log` tablolarini INMA-side'a tasi veya Invekto'da `disconnected_at` ile freeze, (2) `Invekto.Integrations/Services/Zoho/*` kodunu retire (silmeden once 1 release archive), (3) `ZohoSyncService` + `ZohoRetryWorker` + Hangfire job'lari kapat, (4) Dashboard `ZohoConnectionPage` UI'i "INMA uzerinden yonetiliyor" placeholder'a cevir, (5) `appsettings:Zoho:*` secret'lari prod'dan kaldir, (6) DataProtection key ring (`C:\Invekto\Integrations\keys`) uzerindeki Zoho scope'unu purge. **Kaynak doku:** `c:\tmp\zoho-integration-guide-for-inma.md` (INMA'ya verildi, 2026-05-05). **Memory referans:** "INMA chatoperation tek kopru" pattern. Spec yazimi INMA Zoho entegrasyonu DONE olunca. |
