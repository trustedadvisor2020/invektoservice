...
function renderLogError(msg) {
  ...
  if (container) {
    container.innerHTML = `<tr><td colspan="5" class="text-center text-red-400 py-4 text-sm">${escapeHtml(msg)}</td></tr>`;
  }
}
...
function renderLogEntries() {
  ...
  if (_logs.length === 0) {
    container.innerHTML = '<tr><td colspan="5" class="text-center text-slate-500 py-4 text-sm">Log bulunamadi</td></tr>';
    return;
  }
  ...
  container.innerHTML = _logs.map(log => {
    ...
    const color = levelColors[log.Level || log.level] || 'text-slate-400';
    const level = (log.Level || log.level || '-').substring(0, 4).toUpperCase();

    return `
      <tr class="border-b border-slate-800 hover:bg-slate-800/50">
        <td class="py-1 px-2 text-xs text-slate-500 font-mono">${formatTimestamp(log.Timestamp || log.timestamp)}</td>
        <td class="py-1 px-2 text-xs ${color} font-medium">${level}</td>
        <td class="py-1 px-2 text-xs text-slate-400">${escapeHtml((log.Service || log.service || '-').replace('Invekto.', ''))}</td>
        <td class="py-1 px-2 text-xs text-slate-300">${escapeHtml(truncate(log.Message || log.message || '-', 120))}</td>
        <td class="py-1 px-2 text-xs text-slate-500">${escapeHtml(log.Route || log.route || '')}</td>
      </tr>`;
  }).join('');
}
...
export function renderLogsPanel() {
  return `
    <div class="space-y-3">
      ...
      <div class="flex flex-wrap items-center gap-3 bg-slate-800 rounded-lg p-3 border border-slate-700">
        <div class="flex items-center gap-2">
          <label class="flex items-center gap-1 text-xs text-slate-400">
            <input type="checkbox" id="log-level-info" checked class="rounded border-slate-600"> INFO
          </label>
          <label class="flex items-center gap-1 text-xs text-amber-400">
            <input type="checkbox" id="log-level-warn" checked class="rounded border-slate-600"> WARN
          </label>
          <label class="flex items-center gap-1 text-xs text-red-400">
            <input type="checkbox" id="log-level-error" checked class="rounded border-slate-600"> ERR
          </label>
        </div>
        ...
        <input type="text" id="log-search-input" placeholder="Search..."
          class="bg-slate-900 border border-slate-600 rounded px-2 py-1 text-xs text-white w-32 focus:border-blue-500 focus:outline-none"/>
        <select id="log-limit" class="bg-slate-900 border border-slate-600 rounded px-2 py-1 text-xs text-white">
        ...
        <button id="logs-search-btn"
          class="px-3 py-1 text-xs bg-slate-600 hover:bg-slate-500 text-white rounded">Search</button>
        <label class="flex items-center gap-1 text-xs text-slate-400 ml-auto">
          <input type="checkbox" id="logs-auto-toggle" class="rounded border-slate-600"> Auto (5s)
        </label>
      </div>
      <div class="bg-slate-800 rounded-lg border border-slate-700 overflow-hidden">
        <table class="w-full">
          <thead>
            <tr class="bg-slate-900 text-xs text-slate-400">
          ...
          <tbody id="logs-table-body">
            <tr><td colspan="5" class="text-center text-slate-500 py-8 text-sm">Refresh ile loglari yukleyin</td></tr>
          </tbody>
        </table>
      </div>
    </div>`;
