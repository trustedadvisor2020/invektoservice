...
async function generateToken() {
  ...
  if (!tenantId || !userId) {
    if (resultEl) resultEl.innerHTML = '<p class="text-red-600 text-sm">tenant_id ve user_id zorunlu</p>';
    return;
  }
  ...
  try {
    ...
    if (data.error) {
      if (resultEl) resultEl.innerHTML = `<p class="text-red-600 text-sm">${data.error}</p>`;
      return;
    }
    ...
    if (resultEl) {
      resultEl.innerHTML = `<p class="text-emerald-600 text-sm">Token generated. Expires: ${formatTimestamp(data.expires_at)}</p>`;
    }
  } catch (err) {
    ...
    <div class="bg-white rounded-lg p-4 border border-slate-200 shadow-sm">
      ...
      <form id="jwt-form" class="space-y-2">
        ...
        <div class="grid grid-cols-2 gap-2">
          ...
          <div>
            <label class="text-sm text-slate-600">Expiry (dk)</label>
      ...
      <textarea id="jwt-token-display" readonly rows="2"
        class="hidden w-full mt-2 bg-slate-50 border border-slate-300 rounded px-2 py-1.5 text-sm text-emerald-700 font-mono resize-none cursor-pointer"
        onclick="this.select()"></textarea>
      <pre id="jwt-claims-display"
        class="hidden mt-2 bg-slate-50 border border-slate-300 rounded px-2 py-1.5 text-sm text-slate-700 font-mono overflow-x-auto max-h-24"></pre>
    </div>`;
}
