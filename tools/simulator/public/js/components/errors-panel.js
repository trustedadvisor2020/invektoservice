...
function renderErrorMessage(msg) {
  ...
  if (chart) chart.innerHTML = `<p class="text-red-600 text-sm text-center py-4">${escapeHtml(msg)}</p>`;
}
...
function renderErrorChart() {
  ...
  if (buckets.length === 0) {
    chart.innerHTML = '<p class="text-slate-400 text-sm text-center py-8">Hata bulunamadi</p>';
    return;
  }
      ...
      ${buckets.map(b => {
        ...
        return `
          <div class="flex flex-col items-center flex-1 min-w-0" title="${count} errors at ${hour}:00">
            <span class="text-sm text-slate-500 mb-1">${count || ''}</span>
            <div class="w-full bg-red-500/80 rounded-t" style="height: ${height}%"></div>
            <span class="text-sm text-slate-400 mt-1">${String(hour).padStart(2, '0')}</span>
          </div>`;
      }).join('')}
    </div>`;
}
    ...
    <div class="space-y-3">
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
          <span id="error-total" class="text-sm text-slate-500"></span>
      ...
      <div id="error-chart" class="bg-white rounded-lg p-4 border border-slate-200 shadow-sm">
        <p class="text-slate-400 text-sm text-center py-8">Refresh ile hatalari yukleyin</p>
      </div>
    </div>`;
}
