...
function renderScenarioCards() {
  ...
  container.innerHTML = _scenarios.map(s => {
    ...
    return `
      <div class="bg-white rounded-lg p-4 border ${isRunning ? 'border-blue-500 shadow-md' : 'border-slate-200'} shadow-sm">
        <h4 class="text-sm font-medium text-slate-900">${s.name}</h4>
        <p class="text-sm text-slate-500 mt-1">${s.description}</p>
        ${s.note ? `<p class="text-sm text-amber-600 mt-1">${s.note}</p>` : ''}
        <div class="flex items-center justify-between mt-3">
          <span class="text-sm text-slate-400">${s.step_count} adim</span>
          ${isRunning
            ? `<button onclick="window.__stopScenario()" class="px-3 py-1 text-sm bg-red-600 hover:bg-red-700 text-white rounded">Stop</button>`
            : `<button onclick="window.__runScenario('${s.key}')" ${isDisabled ? 'disabled' : ''}
                class="px-3 py-1 text-sm ${isDisabled ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700 text-white'} rounded">Run</button>`
          }
        </div>
      </div>`;
  }).join('');
...
function renderScenarioProgress() {
  ...
  if (_steps.length === 0) {
    container.innerHTML = '<p class="text-slate-400 text-sm text-center py-4">Senaryo calistirin...</p>';
    return;
  }
  ...
  container.innerHTML = _steps.map(step => {
    ...
    const color = statusColors[step.status] || 'text-slate-500';
    ...
    return `
      <div class="flex items-start gap-2 py-1 text-sm">
        ...
        <div class="flex-1">
          ...
          <span class="text-slate-700 ml-1">${escapeHtml(step.message || '')}</span>
          ${step.expected ? `<span class="text-slate-400 ml-1">-- expected: ${step.expected.action || step.expected.description}</span>` : ''}
          ${step.result ? `<span class="text-slate-400 ml-1">(${step.result.status_code}, ${step.result.duration_ms}ms)</span>` : ''}
        </div>
      </div>`;
  }).join('');
}
  ...
  return `
    <div class="space-y-4">
      ...
      <div id="scenario-cards" class="grid grid-cols-2 lg:grid-cols-3 gap-3">
        <div class="bg-white rounded-lg p-4 border border-slate-200 animate-pulse"><div class="h-4 bg-slate-200 rounded w-32"></div></div>
      </div>
      <div class="bg-white rounded-lg p-4 border border-slate-200 shadow-sm">
        <h4 class="text-sm font-medium text-slate-700 mb-2">Execution Log</h4>
        <div id="scenario-progress" class="max-h-64 overflow-y-auto font-mono">
          <p class="text-slate-400 text-sm text-center py-4">Senaryo calistirin...</p>
        </div>
      </div>
    </div>`;
}
