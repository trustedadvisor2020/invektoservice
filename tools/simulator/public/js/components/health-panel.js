...
function renderHealthCards() {
  ...
  container.innerHTML = services.map(s => {
    ...
    const statusColor = s.status === 'ok' ? 'text-emerald-600' : 'text-red-600';

    return `
      <div class="bg-white rounded-lg p-4 border border-slate-200 shadow-sm">
        <div class="flex items-center justify-between mb-2">
          <h4 class="text-sm font-medium text-slate-900">${s.name.replace('Invekto.', '')}</h4>
          <span class="w-3 h-3 rounded-full ${dotClass}"></span>
        </div>
        <p class="${statusColor} text-sm font-medium">${statusText}</p>
        <p class="text-sm text-slate-500 mt-1">${s.url}</p>
        <p class="text-sm text-slate-500">${formatDuration(s.response_time_ms)}</p>
        ${s.error ? `<p class="text-sm text-red-600 mt-1">${s.error}</p>` : ''}
      </div>`;
  }).join('');
}
    ...
    <div class="space-y-3">
      <div class="flex items-center justify-between">
        ...
        <button onclick="document.dispatchEvent(new Event('refresh-health'))"
          class="text-sm text-blue-600 hover:text-blue-700">Refresh</button>
      </div>
      <div id="health-cards" class="grid grid-cols-3 gap-3">
        <div class="bg-white rounded-lg p-4 border border-slate-200 animate-pulse"><div class="h-4 bg-slate-200 rounded w-24"></div></div>
        <div class="bg-white rounded-lg p-4 border border-slate-200 animate-pulse"><div class="h-4 bg-slate-200 rounded w-24"></div></div>
        <div class="bg-white rounded-lg p-4 border border-slate-200 animate-pulse"><div class="h-4 bg-slate-200 rounded w-24"></div></div>
      </div>
    </div>`;
